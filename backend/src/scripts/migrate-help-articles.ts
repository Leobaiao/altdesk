...
async function run() {
    ...
    `);
        ...
        // 3. Seed default global help articles if table is empty
        const countRes = await pool.request().query("SELECT COUNT(*) as cnt FROM altdesk.HelpArticle WHERE TenantId IS NULL");
