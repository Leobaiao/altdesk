...
async function main() {
    try {
        ...
        const convRes = await pool.query("SELECT ConversationId, TenantId FROM altdesk.Conversation WHERE Title LIKE '%leo%'");
        ...
        const connRes = await pool.query("SELECT ConnectorId FROM altdesk.ChannelConnector WHERE Provider = 'GTI' AND IsActive = 1");
        ...
        const query = `
            DELETE FROM altdesk.ExternalThreadMap WHERE ConversationId = '${c.ConversationId}';
        ...
        const verify = await pool.query(`SELECT * FROM altdesk.ExternalThreadMap WHERE ConversationId = '${c.ConversationId}'`);
