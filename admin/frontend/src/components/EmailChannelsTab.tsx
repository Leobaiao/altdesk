...
import { Mail, Plus, Trash2, CheckCircle2, XCircle, RefreshCw, Server, Shield, Send, Inbox, Settings2 } from "lucide-react";
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            {editingChannel && (
                <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }}>
                    <form onSubmit={handleSaveChannel} style={{ background: "var(--bg-secondary)", width: "100%", maxWidth: 800, borderRadius: 20, border: "1px solid var(--border)", display: "flex", flexDirection: "column", maxHeight: "90vh" }}>
                        <div style={{ padding: "24px 32px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            <h3 style={{ margin: 0, fontSize: "1.2rem", fontWeight: 700 }}>Configurar Canal de E-mail</h3>
                            <button type="button" onClick={() => setEditingChannel(null)} style={{ background: "none", border: "none", color: "var(--text-secondary)", cursor: "pointer" }}><Plus size={24} style={{ transform: "rotate(45deg)" }} /></button>
                        </div>
                        ...
                        <div style={{ padding: "32px", overflowY: "auto", display: "flex", flexDirection: "column", gap: 32 }}>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                                <div>
                                    <label style={{ display: "block", marginBottom: 8, fontSize: "0.85rem", color: "var(--text-secondary)" }}>Nome Identificador (ex: Suporte)</label>
                                    <input type="text" value={editingChannel.name} onChange={e => setEditingChannel({...editingChannel, name: e.target.value})} className="settings-input" required />
                                </div>
                                <div>
                                    <label style={{ display: "block", marginBottom: 8, fontSize: "0.85rem", color: "var(--text-secondary)" }}>Endereço de E-mail</label>
                                    <input type="email" value={editingChannel.emailAddress} onChange={e => setEditingChannel({...editingChannel, emailAddress: e.target.value})} className="settings-input" required />
                                </div>
                            </div>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40 }}>
                                ...
                                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                                    <h4 style={{ margin: 0, fontSize: "0.95rem", fontWeight: 600, display: "flex", alignItems: "center", gap: 8 }}><Inbox size={18} color="var(--accent)" /> Entrada (IMAP)</h4>
                                    <div>
                                        <label style={{ display: "block", marginBottom: 6, fontSize: "0.8rem", color: "var(--text-secondary)" }}>Servidor IMAP Host</label>
                                        <input type="text" value={editingChannel.inbound.imapHost} onChange={e => setEditingChannel({...editingChannel, inbound: {...editingChannel.inbound, imapHost: e.target.value}})} className="settings-input" placeholder="imap.gmail.com" required />
                                    </div>
                                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                                        <div>
                                            <label style={{ display: "block", marginBottom: 6, fontSize: "0.8rem", color: "var(--text-secondary)" }}>Porta</label>
                                            <input type="number" value={editingChannel.inbound.imapPort} onChange={e => setEditingChannel({...editingChannel, inbound: {...editingChannel.inbound, imapPort: parseInt(e.target.value)}})} className="settings-input" />
                                        </div>
                                        <div style={{ display: "flex", alignItems: "center", gap: 8, height: "100%", paddingTop: 20 }}>
                                            <input type="checkbox" checked={editingChannel.inbound.imapSecure} onChange={e => setEditingChannel({...editingChannel, inbound: {...editingChannel.inbound, imapSecure: e.target.checked}})} id="imap-ssl" />
                                            <label htmlFor="imap-ssl" style={{ fontSize: "0.8rem" }}>SSL/TLS</label>
                                        </div>
                                    </div>
                                    <div>
                                        <label style={{ display: "block", marginBottom: 6, fontSize: "0.8rem", color: "var(--text-secondary)" }}>Usuário IMAP</label>
                                        <input type="text" value={editingChannel.inbound.username} onChange={e => setEditingChannel({...editingChannel, inbound: {...editingChannel.inbound, username: e.target.value}})} className="settings-input" required />
                                    </div>
                                    <div>
                                        <label style={{ display: "block", marginBottom: 6, fontSize: "0.8rem", color: "var(--text-secondary)" }}>Password IMAP</label>
                                        <input type="password" value={editingChannel.inbound.password} onChange={e => setEditingChannel({...editingChannel, inbound: {...editingChannel.inbound, password: e.target.value}})} className="settings-input" required />
                                    </div>
                                </div>

                                {/* Outbound - SMTP */}
                                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                                    <h4 style={{ margin: 0, fontSize: "0.95rem", fontWeight: 600, display: "flex", alignItems: "center", gap: 8 }}><Send size={18} color="var(--accent)" /> Saída (SMTP)</h4>
                                    <div>
                                        <label style={{ display: "block", marginBottom: 6, fontSize: "0.8rem", color: "var(--text-secondary)" }}>Servidor SMTP Host</label>
                                        <input type="text" value={editingChannel.outbound.smtpHost} onChange={e => setEditingChannel({...editingChannel, outbound: {...editingChannel.outbound, smtpHost: e.target.value}})} className="settings-input" placeholder="smtp.gmail.com" required />
                                    </div>
                                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                                        <div>
                                            <label style={{ display: "block", marginBottom: 6, fontSize: "0.8rem", color: "var(--text-secondary)" }}>Porta</label>
                                            <input type="number" value={editingChannel.outbound.smtpPort} onChange={e => setEditingChannel({...editingChannel, outbound: {...editingChannel.outbound, smtpPort: parseInt(e.target.value)}})} className="settings-input" />
                                        </div>
                                        <div style={{ display: "flex", alignItems: "center", gap: 8, height: "100%", paddingTop: 20 }}>
                                            <input type="checkbox" checked={editingChannel.outbound.smtpSecure} onChange={e => setEditingChannel({...editingChannel, outbound: {...editingChannel.outbound, smtpSecure: e.target.checked}})} id="smtp-ssl" />
                                            <label htmlFor="smtp-ssl" style={{ fontSize: "0.8rem" }}>SSL/TLS</label>
                                        </div>
                                    </div>
                                    <div>
                                        <label style={{ display: "block", marginBottom: 6, fontSize: "0.8rem", color: "var(--text-secondary)" }}>Usuário SMTP</label>
                                        <input type="text" value={editingChannel.outbound.username} onChange={e => setEditingChannel({...editingChannel, outbound: {...editingChannel.outbound, username: e.target.value}})} className="settings-input" required />
                                    </div>
                                    <div>
                                        <label style={{ display: "block", marginBottom: 6, fontSize: "0.8rem", color: "var(--text-secondary)" }}>Password SMTP</label>
                                        <input type="password" value={editingChannel.outbound.password} onChange={e => setEditingChannel({...editingChannel, outbound: {...editingChannel.outbound, password: e.target.value}})} className="settings-input" required />
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style={{ padding: "24px 32px", borderTop: "1px solid var(--border)", display: "flex", justifyContent: "flex-end", gap: 12 }}>
                            <button type="button" onClick={() => setEditingChannel(null)} className="btn btn-secondary" style={{ padding: "10px 24px", borderRadius: 10 }}>Cancelar</button>
                            <button type="submit" className="btn btn-primary" style={{ padding: "10px 32px", borderRadius: 10, display: "flex", alignItems: "center", gap: 8 }}>
